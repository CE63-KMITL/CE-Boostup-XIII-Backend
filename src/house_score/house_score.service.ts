...
export class HouseScoreService {
  ...
  async create(name: string, value: number) {
    ...
    try {
      ...
      return { success: true, message: 'Score created successfully', data: savedScore };
    } catch (error) {
  ...
  async addScore(name: String, value: number) {
    ...
    try {
      ...
      return { success: true, message: 'Score updated successfully', data: updatedScore };
    } catch (error) {
  ...
  async subtractScore(name: string, value: number) {
    ...
    if (score.value - value < 0) {
      console.log(`Cannot subtract from ${name} group as it would result in negative score`);
      throw new HttpException(
        { success: false, message: 'Cannot subtract score, it will result in a negative value' },
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    try {
      const updatedScore = await this.scoreRepository.save(score);
      return { success: true, message: 'Score subtracted successfully', data: updatedScore };
    } catch (error) {
  ...
  async findOne(name: string) {
    ...
    return { success: true, message: 'Score found successfully', data: score };
  }

  // ค้นหาคะแนนบ้านทั้งหมด
  async findAll() {
    const scores = await this.scoreRepository.find();
    return { success: true, message: 'All scores fetched successfully', data: scores };
  }

  // ค้นหาคะแนนทั้งหมดและจัดเรียงตามคำสั่ง ASC DESC
  async findAllSorted(order: 'ASC' | 'DESC') {
    const scores = await this.scoreRepository.find({ order: { value: order } });
    return { success: true, message: 'All scores fetched successfully', data: scores };
  }
  ...
  async update(name: string, value: number) {
    try {
      const score = await this.scoreRepository.findOne({ where: { name } });
      if (!score) throw new HttpException(
        {success: false, message: 'Score not found' },
        HttpStatus.NOT_FOUND,
      );
